...
func (s *Store) LoadManifest(caseID string) (*domain.ReleaseManifest, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.manifestSnapshotCaseID == caseID && s.manifestSnapshot != nil {
		return decodeManifest(s.manifestSnapshot)
	}
	...
	s.manifestSnapshotCaseID = caseID
	s.manifestSnapshot = append([]byte(nil), content...)
	return manifest, nil
}

func decodeManifest(content []byte) (*domain.ReleaseManifest, error) {
