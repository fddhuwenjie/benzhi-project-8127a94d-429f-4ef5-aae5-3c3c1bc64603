...
func (s *Service) GetWorkbench(caseID string) (*Workbench, error) {
	c, err := s.loadWorkbenchCase(caseID)
	if err != nil {
		return nil, fmt.Errorf("组装案件工作台失败: %w", err)
	}
...
func (s *Service) loadWorkbenchCase(caseID string) (*domain.OralHistoryCase, error) {
	c, err := s.store.LoadCase(caseID)
	if err != nil {
		return nil, fmt.Errorf("读取案件快照失败: %w", err)
	}
	return c, nil
}
